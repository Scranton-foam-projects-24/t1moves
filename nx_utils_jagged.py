#Here we're tweaking the nx_utils page for managing rupture networks
...
from polygon import Polygon as PPolygon
...
#old cca definition, isn't good for angles
# def cca(a,b,c):
...
# #test out with nonconvex stuff

# a = [0,2]
# b = [1,1]
# c = [0,0]

# angle = math.atan2(a[0] - b[0], b[1] - a[1])


# angle2 = math.atan2(b[0] - c[0], c[1] - b[1])
...
def network_disorder(vlist, n=-1, areas=None):
    """
    ...
    if n == -2:
        for j in range(len(vlist)):
            ...
            if weighted:
                ...
                #For shard
                if len(vertices) == 2:
                    turn_dists.append( (np.pi/np.sqrt(12))      * areas[j])
                elif len(vertices) == 1:
                    turn_dists.append( (np.pi/(len(vlist[j])*np.sqrt(3))   * areas[j]))
                    
                # if math.isnan(dist):
                #     print(areas[cell])
                #     print('hey!')
                else:
                    turn_dists.append(dist* areas[j])
            else:
                ...
                if len(vertices) == 2:
                    ...
                    turn_dists.append( (np.pi/(len(vlist[j])*np.sqrt(3))) )
                else:
    ...
    if n != -2:
        for j in range(len(vlist)):
            ...
            #comp poly case
            if n == -1:
                comp_poly = poly.regpoly(len(vlist[j]))
                if weighted:
                    
                    if len(vertices) == 2:
                        dist = turning_distance(2,len(vlist[j]))
                        turn_dists.append( dist    * areas[j])
                    elif len(vertices) == 1:
                        turn_dists.append( 0)
                    else:
            ...
            #finally, the six-sided case
                ...
                if weighted:
                    ...
                    if len(vertices) == 2:
                        ...
                        dist = turning_distance(len(vlist[j]),6)
                        turn_dists.append( dist    * areas[j])
                    else:
                        dist, _, _, _ = turning_function.distance(
                            vertices, 
