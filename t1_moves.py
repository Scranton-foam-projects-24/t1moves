...
def generate_gif(num_per_shot,num_t1,duration):
    ...
    snap_dest = '/Users/ryan/Desktop/Python_Programs/T1_Gifs/snaps/'
    if os.path.exists(snap_dest):
        shutil.rmtree(snap_dest)
    os.makedirs(snap_dest)
    ...
    while num_t1_in_gif < num_t1:
        ...
        plt.savefig(str(snap_dest)+str(snap_title))
        ...
        # images.append(str(snap_dest)+str(snap_title))
        snap_num += 1
        plt.close()
    ...
    print(images)
    images[0].save((str(gif_dest)+'test.gif'),format='GIF',append_images=images[0:],save_all=True,duration=duration,loop=0)
...
def compute_cell_areas(pos,cell_major_vertices):
    ...
    for m in range(0,len(cell_major_vertices)): # for each cell
        ...
        areas.append(.5 * sum_of_determinants)
        
    return areas


def histogram_area(areas,H,bin_number):
    
    plt.hist(areas, bins=int(bin_number), edgecolor='black')
    plt.title('Area of Cells')
    plt.xlabel('Area')
    plt.ylabel('Number of Cells')


def histogram_edges(H,bin_number):
    
    edges = []
    for n in range(1,H.number_of_nodes()+1):
        edges.append(len([x for x in H.neighbors(n)]))
...
# generate_gif(snapshot_interval,num_t1,duration=75)

areas = compute_cell_areas(pos,cell_major_vertices)
plt.figure('area')
# bin_number = math.ceil(H.number_of_nodes()/10)
histogram_area(areas, H, 25)

plt.figure('edges')
histogram_edges(H,25)
